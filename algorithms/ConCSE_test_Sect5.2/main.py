...
def main():
    ...
    ## ours and base
    elif args.method == 'ConCSE' and (args.model == 'mbert_uncased' or args.model == 'xlmr_base'):
    ...
    ## simcse and large
        ...
        with open('config_simcse_large.json', 'r') as f:
            train_config = json.load(f)
    ## ours and large
    elif args.method == 'ConCSE' and (args.model == 'xlmr_large'):
    ...
    #     (args.method == 'ConCSE' and args.ours_version is None)):
    #     print("Exiting.")
    #     import sys
    #     sys.exit()
    
    # ConCSE
    if args.method ==  'ConCSE':
        ...
        save_filename = args.schedular + '_warm_' + str(args.warmup_step) + '_lr_' + str(args.lr) + '_margin_' + str(args.margin) + '_triplet_' + str(args.triplet)
        namer = ('seed' + str(args.random_seed) + '_'+args.eval_type + '_'+args.method + '_'
            + args.model +'_'+args.task +'_' +args.lang_type
            + '_temp' + str(args.temp) + args.schedular 
    ...
    if args.run_wandb == True:
        try:
            ...
            ## mbert
            if args.model == 'mbert_uncased' and args.eval_type == 'transfer' and (args.method == 'simcse' or args.method == 'ConCSE'):
                wandb.init(project="mBERT_uncased",
                        name = namer,
                        entity="Put in your ID", config=combined_config)
            
            ## roberta
            if 'xlmr_base' in args.model and args.eval_type == 'transfer' and (args.method == 'simcse' or args.method == 'ConCSE'):
                wandb.init(project="XLM-R_base",
                        name = namer,
                        entity="Put in your ID", config=combined_config)
            
            if 'xlmr_large' in args.model and args.eval_type == 'transfer' and (args.method == 'simcse' or args.method == 'ConCSE'):
